...
print("\n══ 练习题 ══")
print("""
1. 用 matplotlib 绘制正弦和余弦函数（x 从 -2π 到 2π），
   要求：同一图表、不同颜色、图例、网格线、标题、x 轴显示 π 的倍数刻度标签。

2. 给定学生成绩分布数据，绘制：
   a. 直方图（bins=10，显示频率而非频数）
   b. 在同一图表叠加正态分布曲线（用 scipy.stats.norm.pdf）
   c. 箱线图（boxplot），标注异常值

3. 用 subplot 创建 2×2 的图表面板：
   左上：折线图（月度销售趋势）
   右上：柱状图（各品类销量对比，水平方向）
   左下：散点图（价格 vs 销量，点大小代表利润率）
   右下：饼图（市场份额）

4. 绘制热力图（heatmap）：创建 5×5 相关系数矩阵，
   用颜色深浅表示相关性强弱，在每个格子里显示数值。
   （提示：用 imshow 或 seaborn.heatmap）
...
# plt.tight_layout()
# plt.savefig("sin_cos.png", dpi=100)
# plt.close()
