...
print("\n══ 练习题 ══")
print("""
1. 用以下数据创建 DataFrame，完成操作：
   data = {"产品": ["A","B","C","A","B"], "月份": [1,1,1,2,2], "销量": [100,80,60,120,90]}
   a. 按产品分组，计算每个产品的总销量和平均月销量
   b. 新增一列"是否畅销"（销量 >= 90 为 True）
   c. 筛选月份为 2 的所有行，按销量降序排列

2. 加载任意 CSV 文件（或用 pd.read_csv(io.StringIO(csv_str))），
   找出每列的缺失值数量，并用该列的中位数填充数值列的缺失值。

3. 写函数 df_summary(df)，返回每列的：
   数据类型、非空数量、唯一值数量、最大值/最小值（数值列）。

4. 给定两个 DataFrame（学生表和成绩表，用 学号 关联），
   用 merge 实现 SQL 的 LEFT JOIN，找出没有成绩记录的学生。
...
# # a. 分组
# print(df.groupby("产品")["销量"].agg(总销量="sum", 平均月销量="mean"))
# # b. 新列
# df["是否畅销"] = df["销量"] >= 90
# # c. 筛选排序
# print(df[df["月份"] == 2].sort_values("销量", ascending=False))
