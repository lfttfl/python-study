...
print("""
1. 给定包含异常值的数据（自行构造），
   分别用 IQR 法和 Z-score 法检测异常值，比较两种方法的结果差异。

2. 用 pivot_table 完成以下分析：
   data = 订单数据，包含 日期、地区、产品类别、销售额
   a. 行=地区，列=产品类别，值=销售额之和
   b. 计算每个地区各类别的销售额占该地区总销售额的百分比（margins）

3. 用 merge 实现以下 SQL 操作：
   - INNER JOIN：只保留两表都有的记录
   - FULL OUTER JOIN：保留所有记录，缺失值填 NaN
   - SELF JOIN：员工表与自身 join，找出所有员工和他们的直属上司

4. 对时间序列数据：
   a. 用 resample 计算每周的最大值和最小值
   b. 用 rolling(7) 计算 7 日移动平均
   c. 检测并处理时间序列中的缺失日期（reindex + 插值）

参考答案见下方注释：
""")

# # 答案1：IQR vs Z-score 检测
# import numpy as np, pandas as pd
# np.random.seed(42)
# data = pd.Series(np.concatenate([np.random.normal(50, 5, 100), [100, -10, 200]]))
# # IQR 法
...
# print(f"IQR 检测：{len(iqr_outliers)} 个异常值")
# print(f"Z-score 检测：{len(z_outliers)} 个异常值")
